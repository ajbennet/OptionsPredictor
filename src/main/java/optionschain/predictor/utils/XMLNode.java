 ...
 * 
 * The XML Nodes may also be displayed in a JTree, as they extend
...
public class XMLNode extends DefaultMutableTreeNode {
	...
	/**
	 * Suffix used in the toString method This is usefull for appending to the
	 * name of each node when the node is viewed in a JTree. For example, if
	 * want a node with name "books" to to be displayed as "books-2" then set
	 * suffix to be "-2"
	 * 
	 * 
	 */
	...
	/**
	 * Returns short tree path for a node For example aaa,bbb,ccc
	 * 
	 * @return
	 */
	...
	/**
	 * Adds Nodes to an Array, is recursive, and keeps adding the node, then all
	 * its children.
	 */
	...
	/**
	 * Gets an attribute value, throws a RuntimeException if that value does not
	 * exist.
	 * 
	 * @param selected_attribute_name
	 * @return
	...
	public String getAttributeValueNonNull(String selected_attribute_name) {
		...
		if (val == null) {
			throw new RuntimeException(
					new TreePath(this.getPath()) + "  cant find attribute with name " + selected_attribute_name);
		}
	 ...
	 * @param depth
	 *            Locate Elements at this depth.
	 * @param exact
	 *            True, if match is exact. for example, if exact "Return" will
	 *            return only Node "Return" otherwise will return "Return",
	...
	/**
	 * Returns a list of treepaths, whose elements match the string.
	 * 
	 * @param val
	 ...
	 *            name of Element
	 * @param above
	 *            true, if Node is placed before Element.
	 */
	...
	/**
	 * Returns first child with the given name. Throws an Exception, if cant
	 * find child with that name.
	 * 
	 * @param name
	 *            Name of child to return
	...
	/**
	 * Adds node so that it is in alphabetical order with respect to its
	 * siblings.
	 * 
	 * @param newChild
	 */
	...
	/**
	 * Creates a node with a given tree path Path is relative to the root.
	 * 
	...
	public static XMLNode createNewNodewithPath(XMLNode root, String path) {

		{
			...
			if (existing != null) {
				return existing;
				// throw new RuntimeException("we already have node with
				// path>>"+path+"<<");
			} // if
		}
	...
	/**
	 * Returns truncated Name of Node. This is used for generating unique
	 * DocumentID's When we copy a document, the new DocumentID must be unique.
	 * 
	 * We create unique name, by appending a "-1", "-2" etc. If the old
	 * documentName was 30 chars long, we cant append suffix, as Schema defines
	 * that documentName is <=30 chars.
	 * 
	 * hence, we use truncated names. Essentially we remove all vowels from name
	 * to get truncated name.
	 * 
	 */
	...
	/**
	 * Returns set of XMLNodes as an ArrayList Adds all Child nodes including
	 * himself.
	 * 
	 * 
	 * @return
